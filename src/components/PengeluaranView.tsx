...
import { TrendingDown, Plus, Edit2, Trash2, Calendar, CreditCard, Tag, HelpCircle } from 'lucide-react';
...
export default function PengeluaranView({ user, onTransactionChange }: PengeluaranViewProps) {
  const [transactions, setTransactions] = useState<Transaction[]>(() => 
    getUserTransactions(user.id).filter(t => t.type === 'expense')
  );
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (isNaN(numericAmount) || numericAmount <= 0) {
      showFeedback('Masukkan jumlah pengeluaran yang valid (> 0).', 'error');
      return;
    }

    if (!purpose.trim()) {
      showFeedback('Keterangan pengeluaran (Digunakan untuk apa) wajib diisi.', 'error');
      return;
    }
    ...
    if (editId) {
      ...
      showFeedback('Pengeluaran berhasil diperbaharui!', 'success');
    } else {
      // Add Mode
      ...
      showFeedback('Pengeluaran berhasil ditambahkan!', 'success');
    }

    // Refresh views
    const freshTransactions = getUserTransactions(user.id).filter(t => t.type === 'expense');
  ...
  const handleDelete = (id: string) => {
    ...
    const freshTransactions = filtered.filter(t => t.type === 'expense');
    setTransactions(freshTransactions);
    showFeedback('Pengeluaran berhasil dihapus.', 'success');
    onTransactionChange();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-100 dark:border-slate-700 transition-all">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">Pencatatan Pengeluaran</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Pantau ke mana perginya uang Anda dengan mengelompokkan pengeluaran harian.</p>
        </div>
        <div className="flex flex-col shrink-0 text-right bg-rose-50 dark:bg-rose-950/20 px-5 py-3 rounded-2xl border border-rose-100 dark:border-rose-900/50">
          <span className="text-xs font-medium text-rose-600 dark:text-rose-400">Total Pengeluaran Anda</span>
          <span className="text-2xl font-extrabold text-rose-600 dark:text-rose-400" id="expense-total-val">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-xs flex flex-col justify-between">
          <form onSubmit={handleSave} className="p-6 space-y-4">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white border-b border-gray-50 dark:border-slate-700 pb-3 flex items-center gap-2">
              ...
              {editId ? 'Ubah Catatan Pengeluaran' : 'Catat Pengeluaran Baru'}
            </h2>
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                placeholder="Misal: Nasi padang siang, tiket kereta, bayar listrik"
                className="appearance-none rounded-xl block w-full px-4 py-2.5 border border-gray-300 dark:border-slate-600 text-gray-900 dark:text-white bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm transition-all"
                value={purpose}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                >
                  {CATEGORIES_PENGELUARAN.map((cat) => (
            ...
            <div>
              ...
              <textarea
                id="expense-notes"
                rows={2}
                placeholder="Catatan kecil belanjaan..."
        ...
        {/* Right Column: Mini List Expense */}
        <div className="lg:col-span-7 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-6 shadow-xs">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Catatan Pengeluaran Riwayat</h3>
          ...
          {transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-4 border-2 border-dashed border-gray-100 dark:border-slate-700 rounded-2xl">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm font-semibold text-center">Belum ada transaksi pengeluaran.</p>
              <p className="text-gray-400 dark:text-gray-500 text-xs text-center mt-1">Gunakan formulir disamping untuk mencatat pengeluaran pertama Anda.</p>
            </div>
          ) : (
              ...
              {transactions.map((tx) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="h-10 w-10 rounded-lg bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 flex items-center justify-center shrink-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center flex-wrap gap-2">
                        ...
                        <span className="py-0.5 px-2 bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 text-[10px] rounded-full font-bold">
                          {tx.category}
                        </span>
                        <span className="py-0.5 px-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] rounded-full font-bold">
                          {tx.method || 'Cash'}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        ...
                        <span>{tx.date}</span>
                        {tx.notes && (
                          <span className="truncate border-l border-gray-200 dark:border-slate-700 pl-1.5">
                            {tx.notes}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div className="flex items-center gap-1 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-all duration-200">
                      ...
                      <button
                        ...
                        className="p-1.5 bg-white dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-lg border border-gray-100 dark:border-slate-650 text-gray-550 dark:text-rose-405 transition"
                        title="Hapus Catatan"
                      >
